...
import { BookOpen, Code2, Copy, Search, Trash2, X } from "lucide-react";
import { useStore } from "@/lib/store";
import { useModalA11y } from "@/lib/useModalA11y";
import { PROMPT_TEMPLATES } from "@/lib/constants";

const CATEGORIES = ["Tümü", "Kod Yazma", "Hata Ayıklama", "Açıklama", "Refaktöring", "Test"];

/* Snippet kütüphanesi + Prompt şablonları tek modalda, iki sekmede.
   (Eskiden iki ayrı modal/buton vardı — sadeleştirildi.) */
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-line/60 shrink-0">
          <div className="flex items-center gap-1.5 bg-bgsoft/60 p-1 rounded-xl">
            <button
              onClick={() => setTab("snippets")}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-colors ${
                tab === "snippets" ? "bg-brand/15 text-brand font-semibold" : "text-muted hover:text-ink"
              }`}
            >
              <Code2 size={13} /> Snippet&apos;ler
            </button>
            <button
              onClick={() => setTab("prompts")}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-colors ${
                tab === "prompts" ? "bg-brand/15 text-brand font-semibold" : "text-muted hover:text-ink"
              }`}
            >
              <BookOpen size={13} /> Şablonlar
            </button>
          </div>
          <button
            onClick={() => setOpen(false)}
        ...
        {tab === "snippets" ? <SnippetsTab /> : <PromptsTab onPick={() => setOpen(false)} />}
      </div>
    </div>
  );
}
      ...
      <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-muted/40 whitespace-pre-line">
              {snippets.length === 0
...
function PromptsTab({ onPick }: { onPick: () => void }) {
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5">
        {filtered.length === 0 ? (
          <div className="text-center py-8 text-muted text-sm">Aramayla eşleşen prompt bulunamadı.</div>
        ) : (
            ...
            {filtered.map((t) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-sm font-semibold group-hover:text-brand transition-colors">{t.title}</span>
                </div>
                <p className="text-xs text-muted leading-relaxed line-clamp-2">{t.prompt}</p>
              </button>
            ))}
          </div>
        )}
